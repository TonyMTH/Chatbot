...
def get_processed_data(PIK, df, max_seq_len, fasttext_PIK, save_freq=50, process=True):
    if process is True:
        # df = load_data(datafile)
        process_data(df, max_seq_len, PIK, save_freq, fasttext_PIK)
    with open(PIK, "rb") as f:
        data = pickle.load(f)
    data['star'] = data['star'].apply(lambda x: change_star(x))
...
def process_data(df, max_seq_len, PIK, save_freq, fasttext_PIK):
    if not Path(PIK).is_file():
